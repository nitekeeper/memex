...
2. `python3 -m scripts.install` is idempotent and creates `~/.memex/` with `agents.db` and `registry.json`. Auto-invoked by `memex:run` Step 0 on first use; for the install-hardening design rationale (§"Step 0.2") see git history pre-2026-05-26 or query via `memex:run ask`.
