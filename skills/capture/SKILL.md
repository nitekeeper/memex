...
A project is detectable if it contains the path `.ai/wiki/` at its root — that is, a directory named `wiki` inside a directory named `.ai` directly under the project root. Check each root at this fixed path only — do not recurse into subdirectories and do not walk ancestor directories. For example, a folder listed in a VS Code multi-root workspace file counts as a root to check. If no IDE workspace context is available, treat the current working directory as the only root.
...
2. **Check for existing page** at `.ai/wiki/<slug>.md`:
   ...
   - Found and valid → read it; retain all existing frontmatter fields in working state, including any extension fields (e.g. `sources`, `related`, `supersedes`, `archived-reason`). Extract the values of `status`, `created`, and `updated` for the gate display (step 3). Extract `synced-at-commit` (if present) for preservation in step 4 — this field does not appear in the gate display. Then prepare diff description (which fields and body sections change). Format the diff as one line per changed item: list each changed field by name; describe body changes as `body: <section name> updated` — if the body has no named sections, write `body: content updated`. Example: `[UPDATE: title changed, body: rationale section added]`. Do not show full before/after diffs in the gate display (the agent may compute a full diff internally to support the edit/correction path). If the comparison finds no changes to any field (excluding `updated`) and no body changes, note this no-changes state in working memory for the empty-diff guard in step 4. Show `[UPDATE: no content changes detected]` in the gate.
   - Found but malformed (YAML frontmatter cannot be parsed, or one or more required fields — `id`, `title`, `slug`, `status` — are absent or empty) → do not diff against the malformed content. Re-derive all frontmatter fields using step 1 extraction rules as if this were a NEW page — do not carry over any fields from the malformed file, with one exception: scan the raw file text for a line that, after stripping leading and trailing whitespace, starts with `synced-at-commit:` (do this as a plain-text line scan, not via YAML parsing — this extraction must work even when the YAML block cannot be parsed at all). Extract the value as everything after `synced-at-commit:` with leading and trailing whitespace stripped. If the extracted value is empty or whitespace-only, treat this as not found. If such a line is found and the value is non-empty, carry it into working state; write it through to the new file unchanged. The sync skill owns this field and it must never be silently dropped. If no such line is found (or only a whitespace-only value exists), omit the field from the new file. Re-draft the body from the current conversation. Show gate as `[REPAIR: previous write failed — re-drafted from conversation]`. This path is exempt from the empty-diff guard in step 4.
...
3. **Show approval gate** before touching any file:
   ...
   ```
   When constructing the gate: include the `[WARNING]` line only when this is an UPDATE and the existing page has `status: approved`. (The REPAIR path is implicitly exempt — on REPAIR, step 2 re-derives all fields as if NEW, so no existing status value is carried into working state regardless of what the malformed file contained.)
...
4. **On approval**, write the file:
   - `created`: today if NEW or REPAIR; preserve existing value if UPDATE.
   - `updated`: today (YYYY-MM-DD) if content changed; preserve existing value if nothing changed. For this check, exclude the `updated` field itself from the diff comparison. This guard applies only on the UPDATE path — on the NEW path, always proceed to write regardless of diff state. If your content comparison in step 2 found no changes to any field other than `updated`, and no body content changed, do not write the file; tell the user "No changes detected — file not updated." and stop. **Exception: if step 2 flagged this page as a REPAIR path, always proceed to write regardless of diff state** (this exception concerns the guard only — it does not affect what date is written). On the UPDATE path: preserve the existing `updated` value if no content changed (see the empty-diff guard above). If the existing file has no `updated` field and content changed: write today's date. On NEW or REPAIR, always write today's date — there is no existing value to preserve.
   - For `created` and `updated`, use today's date in YYYY-MM-DD format — read from the `# currentDate` value in the session context if present; if absent, determine the platform from the session context (e.g., `Platform:` field) and obtain the date via the Bash tool (`date +%Y-%m-%d`) on POSIX or PowerShell (`Get-Date -Format yyyy-MM-dd`) on Windows; if the platform is unknown, attempt the Bash command first and fall back to PowerShell on failure.
   - `status`: preserve existing value on UPDATE — do not reset to `draft`. On NEW or REPAIR, `status` is `draft` (as derived in step 1).
   - `synced-at-commit`: never set this field. On the UPDATE path: if already present in the file, preserve it unchanged; if not present in the file, omit it from the written output — do not add it. On the REPAIR path: use the value carried into working state from step 2 (the raw-text line scan); if no value was extracted in step 2, omit the field. The sync skill manages this field exclusively.
   - All other frontmatter fields carried in working state from step 2 — including any extension fields such as `sources`, `related`, `supersedes`, and `archived-reason` — write through to the file unchanged. Do not drop fields that are not explicitly named in the rules above.
...
2. **Propose a batch list** before touching any file:
   ...
   ```
   N is the count of remaining candidates at the time this list is (re-)displayed; it is updated each time step 2 is re-entered via step 5.
...
3. **Approve all**: Initialize the written-pages record as an empty list at the start of this step, if it has not already been initialized (it may already exist if re-entering via any path from step 5, or if transitioning from a step 3 intervention — do not reinitialize in any re-entry scenario; the record persists for the full run). Append each page's relative path to the record immediately after it is successfully written and validated (on-demand step 5 completion), if it is not already present in the record. This record is used throughout this run. Run steps 1–5 of on-demand mode for each page (using content extracted in step 1, not fresh user input; do not execute on-demand step 6 — the batch commit in step 6 below replaces it; if the `id` for any page is uncertain during batch execution, derive it from the slug rather than pausing to prompt — flag the derived id in the approval gate so the user can correct it). **Intervenes** means a message received while the agent is executing on-demand steps 1, 2, 4, or 5 for the current page — not while the approval gate (on-demand step 3) is actively awaiting a response (gate responses are handled by on-demand step 3's own branches). On intervention, pause immediately. Check whether the file for the in-progress page exists on disk:
   ...
   - If the file **already exists on disk** for this page (written in this run): present the approval gate showing `[ALREADY WRITTEN — validate only?]` (written this run — the file on disk contains whatever was written; approving triggers on-demand step 5 (validation only — no file re-write); skipping leaves the file on disk unvalidated); if the user approves, run on-demand step 5 (validate); if validation passes, append the file to the written-pages record (if not already present) and continue; if validation fails, remove the page from the written-pages record if it is present (it would be present only if it was written and validated earlier in this run, then re-encountered via this intervention), then proceed to session-end step 5; if the user skips, validate the file immediately (run on-demand step 5) before continuing — do not leave an unvalidated file on disk. If validation passes, do not append the page to the written-pages record — this validation is a safety check only; the user's skip means this page must not be committed in this run. If the page is already present in the written-pages record, remove it — the user's skip means this page must not be committed in this run. If validation fails, remove the page from the written-pages record if it is present (it would be present only if it was written and validated earlier in this run, then re-encountered via this intervention), and proceed to session-end step 5.
...
6. **One commit** for the batch (only when all approved-and-written pages pass validation):
   ...
   Otherwise: stage only the pages in the written-pages record — add each page individually by path. Do not use broad staging commands (`git add .ai/wiki/`, `git add .`, or similar) that could include skipped, unvalidated, or untracked files. Then commit: `wiki: capture session — <W> pages` (apply the em dash encoding check and commit failure handling from `REFERENCE.md` → Commit message formats) where W is the count of distinct pages in the written-pages record for this run (count each page once regardless of how many times it was written or validated; this is different from N in step 2 which is the candidate count). 'This run' spans the full session from initial batch proposal through all re-entries triggered by step 5, ending after all pages have been processed via step 4 or skipped.
...
| Situation | Action |
|---|---|
| `rebuild.py` errors in on-demand mode | Show error, stop, do not commit. Leave file in place for inspection. Tell the user: 'The file has been left on disk for inspection. Fix the issue, then re-invoke the capture skill for the same topic — step 2 will re-read the file: if the frontmatter is invalid or missing required fields, the REPAIR path applies; if the frontmatter is valid and all required fields are present, step 2 treats it as an existing page and shows the normal diff gate.' On re-invocation, no special user action is required beyond invoking capture again. |
| `rebuild.py` errors in session-end mode | See step 5 above — stop, display the current written-pages record as relative paths from the project root, re-enter at step 2. |
...
| Required field missing | Prompt the user for the missing field. If the user provides a value, apply it to the current working state (do not discard previously collected fields) and re-enter at on-demand step 1 to continue extraction. If the user cannot provide the value or declines, stop — do not write anything. Never guess `id`. |
