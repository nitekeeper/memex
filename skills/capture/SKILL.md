...
1. **Extract content** from user input and/or conversation context:
   - `id`: `<project>:<type>:<slug>` — for valid `type` values see `REFERENCE.md` id convention; prompt if uncertain; never guess. After extracting `id`, scan all `.ai/wiki/*.md` files for any file whose frontmatter `id` field matches the extracted id and whose path does not correspond to `<slug>.md`. If such a file is found, apply the `id already exists at a different path` row in the error handling table before proceeding.
...
3. **Show approval gate** before touching any file:
   ...
   ```
   When constructing the gate: include the `[WARNING]` line only when this is an UPDATE and the existing page has `status: approved`.
...
4. **On approval**, write the file:
   ...
   - For `created` and `updated`, use today's date in YYYY-MM-DD format — read from the `# currentDate` value in the session context, or run `date +%Y-%m-%d` (Bash) if the session context value may be stale.
