...
4. **On approval**, write the file:
   ...
   - `synced-at-commit`: never set this field. On the UPDATE path: if already present in the file, preserve it unchanged. On the REPAIR path: use the value carried into working state from step 2 (the raw-text line scan); if no value was extracted in step 2, omit the field. The sync skill manages this field exclusively.
