...
A project is detectable if it has an `.ai/wiki/` subdirectory as a direct child of the current working directory or of any workspace root currently open in the IDE (check each root directly — do not walk ancestor directories). For example, a folder listed in a VS Code multi-root workspace file. If no IDE workspace context is available, treat the current working directory as the only root.
...
5. **Mid-batch validation failure**: if `rebuild.py` fails on any page during a batch run, stop immediately. Do not commit anything. Display the current written-pages record as relative paths from the project root, one per line. This list is your written-pages record — do not reconstruct it from disk; use the record you have been maintaining throughout this run. The page that just failed validation is not in this record — it was never appended because append requires successful validation. Do not add it to the record at this point. Resolution means one of:
...
| Situation | Action |
|---|---|
| `rebuild.py` errors in on-demand mode | Show error, stop, do not commit. Leave file in place for inspection. Tell the user: 'The file has been left on disk for inspection. Fix the issue, then re-invoke the capture skill for the same topic — step 2 will re-read the file: if the frontmatter is invalid or missing required fields, the REPAIR path applies; if the frontmatter is valid, step 2 treats it as an existing page and shows the normal diff gate.' On re-invocation, no special user action is required beyond invoking capture again. |
