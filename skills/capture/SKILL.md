...
2. **Check for existing page** at `.ai/wiki/<slug>.md`:
   ...
   - Found but malformed (YAML frontmatter cannot be parsed, or one or more required fields — `id`, `title`, `slug`, `status` — are absent or empty) → do not diff against the malformed content. Re-derive all frontmatter fields using step 1 extraction rules as if this were a NEW page — do not carry over any fields from the malformed file, with one exception: if the malformed file contains a `synced-at-commit` field that can be read (even if other fields are absent or the YAML is otherwise unparseable), carry it into working state and write it through to the new file unchanged (the sync skill owns this field and it must never be silently dropped). Re-draft the body from the current conversation. Show gate as `[REPAIR: previous write failed — re-drafted from conversation]`. This path is exempt from the empty-diff guard in step 4.
